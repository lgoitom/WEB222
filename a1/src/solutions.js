...
function toCamelCase(name, uppercase) {
  ...
  if (uppercase === null || uppercase !== true) {
    ...
    if (name.includes('-')) {
      for (i = 0; i < splitted.length; i++) {
        splitted[i] = splitted[i].charAt(0).toUpperCase() + splitted[i].slice(1);
        converted = splitted[0] + splitted[i];
      }
    } else {
...
function createLinkTag(rel, href) {
  ...
  if (rel.includes(' ') || rel.includes('\t')) {
    counter = rel.trim().split(/\s+/).length;
  ...
  if (href.includes(' ') || href.includes('\t')) {
    counter = href.trim().split(/\s+/).length;
  ...
  linkTag = `<link rel=${getRel} href=${getHref}>`;
  return linkTag;
}
...
function parseDateString(value) {
  ...
  if (typeof value !== 'string' || value === null || value === '' || value.length !== 10) {
    throw new Error(
  ...
  if (
    (value.indexOf('/') === 4 && value.lastIndexOf('/') === 7) ||
    ...
    dateSet.setDate(indices[2]);
    dateSet.setMonth(indices[1] - 1);
    dateSet.setFullYear(indices[0]);
    return dateSet;
  } else {
    throw new Error(
      'invalid date string, expected a `YYYY/MM/DD` or `DD/MM/YYYY` formatted string'
...
function toDateString(value, format) {
  ...
  splitter = String(value).split(' ').splice(1, 3);

  console.log(splitter[0]);

  if (splitter[0] === 'Jan') {
    month = '01';
  } else if (splitter[0] === 'Feb') {
    month = '02';
  } else if (splitter[0] === 'Mar') {
    month = '03';
  } else if (splitter[0] === 'Apr') {
    month = '04';
  } else if (splitter[0] === 'May') {
    month = '05';
  } else if (splitter[0] === 'Jun') {
    month = '06';
  } else if (splitter[0] === 'Jul') {
    month = '07';
  } else if (splitter[0] === 'Aug') {
    month = '08';
  } else if (splitter[0] === 'Sep') {
    month = '09';
  } else if (splitter[0] === 'Oct') {
    month = '10';
  } else if (splitter[0] === 'Nov') {
    month = '11';
  } else if (splitter[0] === 'Dec') {
    month = '12';
  }

  if (format === 'YYYY/MM/DD') {
    year = splitter[2];
    day = splitter[1].replace(',', '');
 ...
 *
 * A digital clock displays time in the format "HH:MM:SS". However, in a dateSet
...
function normalizeTime(value) {
  if (value.valueOf() === '' || value.includes('AM') || value.includes('PM')) {
    let timeString = value;
    let [hour, minute, second] = timeString.split(':');

    if (hour > 12 && (timeString.includes('PM') || timeString.includes('AM'))) {
      return null;
    } else {
      if ((_minute) => 60) {
        return null;
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  ...
  const urlResourceType = encodeURIComponent(resourceType);
  const urlResourceId = encodeURIComponent(resourceId);
  const urlExtraData = extraData ? '/' + encodeURIComponent(extraData) : '';

  const api = `/${urlResourceType}/${urlResourceId}${urlExtraData}`;
